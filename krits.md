...
Суть цього критерію полягає у виборі альтернативи, виходячи з найбільш імовірного стану середовища $$s^\star \in S: s^\star = \arg\max\limits_{s \in S} p(s)$$. При використанні цього критерію ОПР вважає, що середовище знаходиться у стані $$S^\star$$ і вибирає альтернативу з критерію $$E_{MOD}(x) = \max\limits_{x \in X} f(x, s^\star)$$, тобто з умови
